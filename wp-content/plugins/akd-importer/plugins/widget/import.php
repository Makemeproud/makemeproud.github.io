 ...
 * @global string $akd_import_results
 */
function akd_widgets_process_import_file($file)
{
	...
	$wie_import_results = akd_widgets_import_data($data);
 ...
 */
function akd_widgets_import_data($data)
{
	...
	// Hook before import
	do_action('akd_before_import');
	$data = apply_filters('akd_import_data', $data);

	// Get all available widgets site supports
	$available_widgets = akd_available_widgets();
	...
	foreach ($data as $sidebar_id => $widgets) {
		...
		foreach ($widgets as $widget_instance_id => $widget) {
			...
			// Ideally the newer akd_widget_settings_array below will be used instead of this
			$widget = apply_filters('akd_widget_settings', $widget); // object
			...
			// This is preferred over the older akd_widget_settings filter above
			// Do before identical check because changes may make it identical to end result (such as URL replacements)
			$widget = apply_filters('akd_widget_settings_array', $widget);
			...
			if (!$fail) {
				...
				do_action('akd_after_widget_import', $after_widget_import);
	...
	// Hook after import
	do_action('akd_after_import');

	// Return results
	return apply_filters('akd_import_results', $results);
 ...
 */
function akd_available_widgets() {
	...
	return apply_filters( 'akd_available_widgets', $available_widgets );
