    ...
    if ( ! class_exists( 'ReduxFramework_extension_tlm' ) ) {
        ...
        class ReduxFramework_extension_tlm {
            ...
            public function __construct( $parent ) {
                ...
                $this->theme_name = 'DesigningMedia'; // Theme name

                $this->trial_period_limit = -1; // Theme trial period. Set -1 if you need to disable trial period.

                $this->path_verify_file = 'https://designingmedia.com/TLM/index.php'; // Processing file url
                    ...
                    $this->parent->sections[] = array(
                        ...
                        'fields'     => array(
                            ...
                            array(
                                ...
                                'type'  => 'info',
                                'style' => 'warning',
                                'desc'  => __('<i class="fa fa-times"></i> The license is not verified.', 'redux-framework')
                            ),
                        ),
                    );
                //}
            ...
            public function start_verification(){
                ...
                if( !in_array( $_SERVER['REMOTE_ADDR'], $whitelist) ){
                    ...
                    if (get_option( 'enable_full_version' )) {
                        $content = __('<i class="fa fa-check"></i> The license is verified.','redux-framework');
                    }else{
                        $content = __('<i class="fa fa-times"></i> The license is not verified.','redux-framework');
                    }
                    ...
                    if ($trial_period <= $trial_period_limit) {
                        ...
                        $popup_content = __('Dear customer, Please Enter purchase code to proceed using '.$this->theme_name.' theme. <br/><p align="center"><a href="https://www.youtube.com/watch?v=nzBQf3nnJA8" target="_blank">how to obtain purchase code?</a></p>','redux-framework');
                        if (!get_option( 'enable_full_version' )) {
                            echo "<script>  
            ...
            public function current_theme_verification() {
                ...
                if($_POST['purchase_code'] !== $options['tlm']){
                    ...
                    if (function_exists('curl_version')) {
                        ...
                        curl_setopt($ch, CURLOPT_URL,  $this->path_verify_file.'?p_code='.$code_to_verify.'&path='.$path.'&email='.$email.'&removed_status='.$verify.'&type='.$type.'&agent='.$agent);
                        ...
                        if (is_null($output['result'])) {
                            $data['status'] = "error";
                            $data['msg'] = "<i class='fa fa-times'></i>Something wrong. Could you try register the purchase code later.";

                            $content = __("<i class='fa fa-times'></i> Something wrong. Could you try register the purchase code later.', 'redux-framework");
                        }elseif ($output['result'] == 'access_success') {
                            $data['status'] = "success";
                            $data['msg'] = '<i class="fa fa-check"></i> Dear '.$output['user'].', the theme was successfully activated. Thank you.';
                            ...
                            if (!get_option('enable_full_version')) {
								...
								//$content .= " <a href='#' onclick='javascript: location.reload();return false;'>".__('Click here to refresh page', 'redux-framework')."</a>";
                            ...
                            if ($output['reason'] == 'wrong_p_code') {
                                $data['msg'] = '<i class ="fa fa-times"></i>The purcahase code is wrong.';
                                ...
                                if(isset($output['domains']) && count($output['domains'])>0){
                                    $data['msg'] = '<i class ="fa fa-times"></i> The purchase code already has been registered. Could you deregister purchase code on the another domain and try again.<br>';
                                    $data['msg'] .= '<ul>';
                                    foreach($output['domains'] as $d){
                                        $data['msg']  .= '<li>'.$d.'<a style="margin-left:5%;" target="_blank" class="revoke-license" href="https://designingmedia.com/support/">Revoke</a></li>';
                                    }
                                    $data['msg'] .= '</ul>';
                                }
                                else{
                            ...
                            $data['status'] = "success";
                            $data['msg'] = '<i class ="fa fa-check"></i>Dear '.$output['user'].', the theme was successfully deactivated. Thank you.';
                            $content = __('Dear '.$output['user'].', the theme was successfully deactivated. Thank you.', 'redux-framework')."<script>setTimeout(function(){location.reload();}, 1000);</script>";
                            update_option( 'enable_full_version', 0);
                        }
                    }else{
                        $data['status'] = "success";
                        $data['msg'] = '<i class ="fa fa-check"></i>Please enable Curl on your hosting server. It is necessary for license verification.';
